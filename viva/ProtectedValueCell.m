...
//	height -= 8.0;
	theSize->width = /*[fontObj getWidthOf:[self stringValue]] +*/ [fontObj getWidthOf:"m"] * 10;
...
- drawInside:(const NXRect *)rect inView:view
{
	...
	NXTextFontInfo(fontObj, &ascender, &descender, &lineHeight);
	theRect.size.width = /*[fontObj getWidthOf:[self stringValue]] +*/ [fontObj getWidthOf:"m"] * 10;
	theRect.origin.y += descender; //(descender/3.0);
	[[mySubView setMonoFont:YES] setAlignment:NX_RIGHTALIGNED];
	[mySubView selectAll:self];
	[mySubView setSelProp:NX_RIGHTALIGN to:0];
	[mySubView selectNull];
